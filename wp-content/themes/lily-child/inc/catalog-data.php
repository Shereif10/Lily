...
/**
 * Product IDs assigned to one canonical term or any of its color shades.
 *
 * Kept for admin counts; the storefront relies on the automatic Parent Color
 * assignment above, so native WooCommerce queries already match correctly.
 *
 * @param string $taxonomy Taxonomy name.
 * @param int    $term_id  Term ID.
 * @return int[]
 */
function lily_term_product_ids( $taxonomy, $term_id ) {
	$term_id = absint( $term_id );
	...
	$ids   = array( $term_id );
	$ids   = array_merge( $ids, 'pa_color' === $taxonomy ? lily_color_descendant_ids( $term_id ) : array() );
	$query = new WP_Query(
		array(
			'post_type'              => 'product',
			'post_status'            => 'publish',
