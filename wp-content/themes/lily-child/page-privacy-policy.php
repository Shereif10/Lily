<?php
/**
 * Lily Privacy Policy page (slug: privacy-policy).
 *
 ...
 * (Privacy Page tab, same settings system as the other Lily pages) with
 * the complete approved bilingual Privacy content as fallback.
 *
 * @package Lily
 */
...
add_filter(
	...
	static function ( $classes ) {
		...
		$classes[] = 'lily-privacy-policy-page';
		return $classes;
	}
);
...
$lily_privacy_approved = function_exists( 'lily_privacy_approved_content' )
	? lily_privacy_approved_content()
	: array(
...
$lily_hero_title = lily_get_option( 'privacy_hero_title', isset( $lily_privacy_approved['hero_title_en'] ) ? $lily_privacy_approved['hero_title_en'] : __( 'Privacy Policy', 'lily' ) );
$lily_hero_intro = lily_get_option( 'privacy_hero_intro', isset( $lily_privacy_approved['hero_intro_en'] ) ? $lily_privacy_approved['hero_intro_en'] : '' );
...
$lily_sections = array();
for ( $lily_i = 1; $lily_i <= 21; $lily_i++ ) {
	$lily_fallback = isset( $lily_privacy_approved['sections'][ $lily_i - 1 ] ) && is_array( $lily_privacy_approved['sections'][ $lily_i - 1 ] )
		? $lily_privacy_approved['sections'][ $lily_i - 1 ]
		: array();

	$lily_sections[] = array(
		'num'   => sprintf( '%02d', $lily_i ),
		'title' => lily_get_option( "privacy_{$lily_i}_title", isset( $lily_fallback['title_en'] ) ? $lily_fallback['title_en'] : '' ),
		'body'  => lily_get_option( "privacy_{$lily_i}_body", isset( $lily_fallback['body_en'] ) ? $lily_fallback['body_en'] : '' ),
	);
}
?>
...
<main id="primary" class="site-main lily-terms">
	...
	<section class="lily-terms-band lily-terms-policy" aria-label="<?php esc_attr_e( 'Privacy policy details', 'lily' ); ?>">
